setwd("/gpfs/ycga/project/wang_zuoheng/cc2747/VACS/normal200522/Logit_Tskew")
library(foreach)
library(doMC)
library(doRNG)
library(sn)
test.fun = function(rho=1,P_1000=0.05, alpha_g=log(1.5), alpha_x=log(1.5), alpha_gx=log(1.2),
  ...
  Mean_X  = Mean_X0*(1-P_D) + Mean_X1 * P_D
  X = Mean_X+rnorm(N)
  if (rho==1) {
    Xm=X
  } else {
...
test.fun.main=function(rep=100,rho=0.5,P_1000=0.05, alpha_g=log(1.5), alpha_x=log(1.5), alpha_gx=log(1.0),
  ...
  return(c(IE_log=mean(res[,1] < 0.05),IE_rev=mean(res[,2] < 0.05),
           IE_log=mean(res[,1] < 0.0001),IE_rev=mean(res[,2] < 0.0001),
