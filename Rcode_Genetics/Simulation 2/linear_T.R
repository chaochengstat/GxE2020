setwd("/gpfs/ycga/project/wang_zuoheng/cc2747/VACS/normal200522/Linear_Tskew")
library(foreach)
library(doMC)
library(doRNG)
library(sn)
test.fun = function(rho=1,P_1000=0.05, alpha_g=log(1.5), alpha_x=log(1.5), alpha_gx=log(1.2),
  ...
  Mean_X  = d4+beta_d*D+beta_gd*G*D
  X = Mean_X+rnorm(N)
...
test.fun.main=function(rep=100,rho=0.5,P_1000=0.05, alpha_g=log(1.5), alpha_x=log(1.5), alpha_gx=log(1.0),
  ...
  return(c(IE_log1=mean(res[,1] < 0.05),IE_rev1=mean(res[,2] < 0.05),
           IE_log2=mean(res[,1] < 0.0001),IE_rev2=mean(res[,2] < 0.0001),
           Chi2_ratio=mean(res[,4])/mean(res[,3])))
}
...
for (i in (subs)) {
  ...
  res2[i,]=c(test1[3:4],test2[3:4],test3[3:4],test4[3:4])
  par.mat[i,]=c(P_1000,alpha_g,Cor_XZ2,Cor_XG)
  print(paste("NO.",i,"and",Sys.time()))
  print(res1[i,])
}
file=paste("type_1_error_skew_",aid,".csv",sep="")
write.csv(res1,file)

file=paste("type_1_error_skew_small",aid,".csv",sep="")
write.csv(res2,file)
